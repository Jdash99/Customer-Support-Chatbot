...
def memory_config(mode, for_update=False):
    """Memory block for create/update.

    By default the harness attaches a MANAGED memory that persists across
    runtimeSessionIds: a brand-new session can recall tickets filed in an
    earlier conversation and claim they already exist. Session state *within*
    one runtimeSessionId is handled by the runtime itself and is unaffected,
    so we disable the cross-session memory.
    """
    block = {"disabled": {}} if mode == "disabled" else {"managedMemoryConfiguration": {}}
    # UpdateHarness wraps the same union in optionalValue.
    return {"optionalValue": block} if for_update else block
...
def main():
    ...
    parser.add_argument("--memory", choices=("disabled", "managed"),
                        default="disabled",
                        help="Cross-session memory. 'disabled' (default) keeps "
                             "each runtimeSessionId independent — turns inside a "
                             "session are still remembered, which is all this "
                             "project needs. 'managed' lets the harness recall "
                             "PAST sessions, which contaminates the eval suite.")
    ...
    if existing:
        ...
        acc.update_harness(
            ...
            memory=memory_config(args.memory, for_update=True),
        )
    else:
        ...
        for attempt in range(3):
            try:
                harness = acc.create_harness(
                    ...
                    model=model_config(args.model),
                    systemPrompt=[{"text": prompt}],
                    memory=memory_config(args.memory),
                )
                break
