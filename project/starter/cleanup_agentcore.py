...
def main():
    ...
    # 3. Guardrail (created by setup_guardrail.py, if you ran it)
    guardrail_id = config.get("guardrail_id")
    if guardrail_id:
        print(f"Deleting guardrail {guardrail_id}...")
        br = boto3.client("bedrock", region_name=config["region"])
        try:
            # Deleting the guardrail removes all of its versions too.
